...
يفتح المتصفح تلقائياً. تدفق الروابط من خطوتين: **فحص الفيديو أولاً** (يقرأ العنوان والمدة والجودات الموجودة فعلياً فيه) ثم تختار جودة حقيقية وتستخرج. حد الرفع 4GB (قابل للزيادة في `.streamlit/config.toml`).

يمكن نشره مجاناً على [Streamlit Community Cloud](https://streamlit.io/cloud). ملاحظة: يوتيوب يقاوم التحميل من خوادم السحابة؛ التطبيق يجرّب تلقائياً 6 عملاء ثم مرايا Invidious/Piped، ويدعم `cookies.txt` وبروكسي عبر secret باسم `YTDLP_PROXY` — وعلى جهازك تعمل الروابط مباشرة.
